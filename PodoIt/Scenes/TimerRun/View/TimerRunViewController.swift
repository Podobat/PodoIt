  ...
  private func bind() {
    ...
    viewModel.isMuteDriver // 음소거(mute)의 Bool 상태 (아래는 tick 모음이라 계속 호출되서 분리)
      .drive(with: self) { vc, isMute in
        ...
        if isMute { // 음소거 상태
          vc.showToastBelow("알림이 꺼졌어요.", icon: UIImage(named: "circle-bang"), above: vc.animationSectionView)
        } else { // 음소거 아닌 상태
          vc.showToastBelow("알림이 켜졌어요.", icon: UIImage(named: "circle-check-green"), above: vc.animationSectionView)
        }
